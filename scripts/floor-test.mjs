...
import { MAX_SEATS, START_TABLE, HORSES } from "../src/casino-core.js";
...
await say("p1", { type: "FLOOR_BET", betType: "win", picks: ["king"], stake: 20 });
...
ok("the copy keeps the same picks", floor.f.bets[1].type === "win" && floor.f.bets[1].picks[0] === "king");
ok("but pays for it themselves", floor.f.players.p2.table === START_TABLE - 10);

await say("p3", { type: "FLOOR_BET", betType: "long", picks: ["queen"], stake: 5 });
...
await say("p3", { type: "FLOOR_BET", betType: "win", picks: ["jack"], stake: 99999 });
...
await say("p2", { type: "FLOOR_BET", betType: "win", picks: ["jack"], stake: 5 });
...
ok("a result goes out", !!result && result.order.length === HORSES.length);
...
for (let i = 0; i < MAX_SEATS + 3; i++) {
  const uid = `x${i}`;
  floor.f.players[uid] = { uid, name: `X${i}`, table: 50, tokens: 1, staked: true };
